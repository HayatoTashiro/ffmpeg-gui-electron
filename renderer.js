...
document.addEventListener('DOMContentLoaded', () => {
  document.getElementById('select-input-button').addEventListener('click', selectInput);
  document.getElementById('select-output-button').addEventListener('click', selectOutput);
  document.getElementById('start-button').addEventListener('click', startConversion);
});
...
function adjustTime(id, delta) {
  ...
  let [h, m, s] = (input.value || '00:00:00').split(':').map(n => parseInt(n) || 0);
